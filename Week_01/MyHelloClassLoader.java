...
public class MyHelloClassLoader extends ClassLoader {
    public static void main(String[] args) {
        try {
            Class<?> newClass = new MyHelloClassLoader().findClass("Hello");
            Object obj = newClass.newInstance();
            Method method = newClass.getMethod("hello");
            method.invoke(obj); // 通过反射调用其方法
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        ...
        for (int i = 0; i < bytes.length; ++i) { // 将字节码转义为 255 - x
